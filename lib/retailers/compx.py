...
class Compx(Retailer):
    ...
    def get_prices(self, search_keyword: str) -> list[tuple[str,float]]:
        request = requests.Request('GET','https://compx.com.ua/katalog/search/filter/page=all;sort_price=ASC/', params={'q':get_query_string_for_search_keyword(search_keyword)}, )
        ...
        for item_main_div in parser.find_all(class_='catalogCard-main'):
            ...
            price = item_main_div.find(class_='catalogCard-price').text.strip().replace(' ','').replace('грн','')
